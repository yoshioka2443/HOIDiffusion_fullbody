...
class dataset_dex():
    def __init__(self, path_json, reg_path):
        ...
        with open(path_json, 'r', encoding='utf-8') as fp:
            ...
            for file in data:
                ...
                self.files.append({'rgb': image_path, "bodymesh": bodymesh_path, "mask": mask_path, "sentence":file["sentence"], "seg":seg_path, "shape":{"top":file["top"],"bottom":file["bottom"],"left":file["left"],"right":file["right"]}})
    ...
    def __getitemHOI__(self, idx):
        ...
        # Read bodymesh image (3 channels)
        bodymesh = cv2.imread(file["bodymesh"])
        bodymesh = bodymesh[top_point:bottom_point, left_point:right_point]
        bodymesh = cv2.copyMakeBorder(bodymesh, top, bottom, left, right, cv2.BORDER_CONSTANT, value=[0,0,0])
        bodymesh = cv2.resize(bodymesh, (512, 512))
        bodymesh = img2tensor(bodymesh, bgr2rgb=True, float32=True) / 255.
        ...
        return {"data":{'im': im, 'depth': depth, "bodymesh": bodymesh, 'sentence': sentence, "seg": seg, "mask": mask, 'shape':file["shape"]}}
