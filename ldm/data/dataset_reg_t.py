...
class dataset_dex():
    def __init__(self, path_json, reg_path):
        ...
        with open(path_json, 'r', encoding='utf-8') as fp:
            ...
            for file in data:
                ...
                self.files.append({'rgb': image_path, "skeleton":skeleton_path, "mask": mask_path, "sentence":file["sentence"], "seg":seg_path, "texture":texture_path, "shape":{"top":file["top"],"bottom":file["bottom"],"left":file["left"],"right":file["right"]}})
    ...
    def __getitemHOI__(self, idx):
        ...
        texture = cv2.imread(file["texture"])
        texture = texture[top_point:bottom_point, left_point:right_point]
        texture = cv2.copyMakeBorder(texture, top, bottom, left, right, cv2.BORDER_CONSTANT, value=[0,0,0])
        texture = cv2.resize(texture, (512, 512))
        texture = img2tensor(texture, bgr2rgb=True, float32=True) / 255.

        sentence = file['sentence']

        # return {"data":{'im': im, 'depth': depth, 'skeleton':skeleton, 'sentence': sentence, "seg": seg, "mask": mask, 'shape':file["shape"]}}
        return {"data":{'im': im, 'depth': depth, 'skeleton':skeleton, 'sentence': sentence, "seg": seg, "mask": mask, "texture": texture, 'shape':file["shape"]}}
