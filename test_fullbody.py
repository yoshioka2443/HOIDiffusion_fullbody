...
from ldm.modules.extra_condition.api_fullbody import (ExtraCondition, get_adapter_feature, get_cond_model)
from ldm.data.dataset_grabnet_fullbody import dataset_grabnet
...
def main():
    ...
    with torch.inference_mode(), \
        ...
        for _, batch in tqdm(enumerate(train_dataloader)):
            ...
            adapter_features, append_to_context = get_adapter_feature(batch["bodymesh"].half().cuda(), depth.half().cuda(), batch["mask"].half().cuda(), adapter)
