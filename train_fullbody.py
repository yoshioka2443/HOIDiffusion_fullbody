...
from ldm.data.dataset_reg_fullbody import dataset_dex
import argparse
...
if __name__ == '__main__':
    ...
    # print(f"Process {opt.local_rank} is using GPU {torch.cuda.current_device()}")
    ...
    # wandb init
    wandb.login()
    wandb.init(project="fullbody", name=opt.name)
    ...
    for epoch in tqdm(range(start_epoch, opt.epochs)):
        ...
        for _, data in tqdm(enumerate(train_dataloader)):
            ...
            features_adapter = model_ad(bodymesh, normal, mask)
